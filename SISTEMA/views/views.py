...
class ExportarReportesView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        for vendedor in vendedores:
            reportes = InformeVendedor.objects.filter(
                vendedor=vendedor,
                fecha_ingreso__date__range=[f_inicio, f_termino]
            ).select_related('cliente', 'metodo_pago').prefetch_related('tipo_producto').order_by('fecha_ingreso')

            if reportes.exists():
                data_reporte.append({'vendedor': vendedor, 'reportes': reportes})
    ...
    def generar_excel(self, data_reporte, f_inicio, f_termino):
        ...
        for row in range(3, 6):
            ...
            ws_stats[f'A{row}'].fill = PatternFill(start_color="E2EFDA", fill_type="solid")
...
def error_404_view(request, exception):
    return render(request, 'Error.html', {'mensaje': 'La página que buscas no existe o ha sido movida.'}, status=404)

def error_500_view(request):
    return render(request, 'Error.html', {'mensaje': 'Tuvimos un problema interno. El equipo técnico ha sido notificado.'}, status=500)
    
@login_required
def home(request):
    ultimos_reportes = InformeVendedor.objects.filter(usuario=request.user).order_by('-fecha_inicio')[:3]
    return render(request, 'SISTEMA/home.html', {'ultimos_reportes': ultimos_reportes})